...
def detect_patterns(log_entries: List[Dict[str, Any]], min_similarity: float = 0.7) -> Dict[str, Any]:
    """
    ...
    error_warning_logs = [
        ...
        if entry.get('log_level') in ['ERROR', 'WARNING']
    ]
...
def extract_patterns(log_entries: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if status_codes:
        ...
        patterns.append({
            ...
            "examples": [{"code": code, "count": count} for code, count in status_counter.most_common(5)]
        })
    ...
    if endpoints:
        ...
        patterns.append({
            ...
            "count": len(endpoints),
            "examples": [{"endpoint": ep, "count": count} for ep, count in endpoint_counter.most_common(5)]
        })
    ...
    if exceptions:
        ...
        patterns.append({
            ...
            "examples": [{"type": exc, "count": count} for exc, count in exception_counter.most_common(5)]
        })
    
    return patterns

def group_similar_errors(log_entries: List[Dict[str, Any]], min_similarity: float = 0.7) -> List[Dict[str, Any]]:
    """
