...
class AIService:
    ...
    def analyze_logs(self, analysis_data: Dict[str, Any], log_sample: list = None) -> Dict[str, Any]:
...
"""
            ...
            response = self.openai_client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "Sen bir log analiz uzmanısın. Log dosyalarını analiz edip yorum yapıyorsun."},
    ...
    def _create_summary(self, analysis_data: Dict[str, Any], log_sample: list = None) -> str:
        ...
        if top_errors:
            ...
            for i, error in enumerate(top_errors[:5], 1):
                summary_lines.append(f"  {i}. {error.get('message', '')[:100]} ({error.get('count', 0)} kez)")
