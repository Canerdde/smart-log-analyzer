...
def export_analysis_to_pdf(
    ...
    file_data = [
        ...
        ["Yüklenme Tarihi", log_file.uploaded_at.strftime("%Y-%m-%d %H:%M:%S") if log_file.uploaded_at else ""],
    ...
    if analysis.top_errors:
        ...
        error_table = Table(error_data, colWidths=[0.5*inch, 3.5*inch, 1*inch, 1*inch])
    ...
    entry_table = Table(entry_data, colWidths=[0.6*inch, 0.8*inch, 1*inch, 3.6*inch])
...
def export_analysis_to_excel(
    ...
    ws[f'B{row}'] = log_file.uploaded_at.strftime("%Y-%m-%d %H:%M:%S") if log_file.uploaded_at else ""
    ws[f'A{row}'].font = Font(bold=True)
    row += 1
    ...
    if entries:
        ...
        for idx, entry in enumerate(entries[:5000], 2):
            ...
            ws2.cell(row=idx, column=3).value = entry.timestamp.strftime("%Y-%m-%d %H:%M:%S") if entry.timestamp else ""
...
def export_logs_to_json(
    ...
    data = {
        "file_info": {
            ...
            "uploaded_at": log_file.uploaded_at.isoformat() if log_file.uploaded_at else None,
    ...
    if include_analysis and analysis:
        ...
        if analysis.top_errors:
            ...
            for error in analysis.top_errors:
                ...
                ET.SubElement(error_elem, "percentage").text = str(error.get("percentage", 0))
        ...
        if analysis.top_warnings:
            ...
            for warning in analysis.top_warnings:
                ...
                ET.SubElement(warning_elem, "percentage").text = str(warning.get("percentage", 0))
