        ...
        try:
            ...
            async with httpx.AsyncClient(auth=auth, timeout=10.0) as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(
                        f"Jira issue oluşturma hatası: {response.status_code} - {response.text}"
                    )
                    return None
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(
                        f"Trello card oluşturma hatası: {response.status_code} - {response.text}"
                    )
                    return None
        except Exception as e:
...
{chr(10).join([f"- {entry.get('message', '')[:200]}" for entry in log_entries[:5]])}
        ...
        return await TrelloIntegration.create_card(
            ...
            name,
            description,
            labels=["red"],  # Kırmızı label (alert)
        )
