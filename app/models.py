from sqlalchemy import Column, Integer, String, DateTime, Text, ForeignKey, Float, JSON, Boolean, Table
...
class User(Base):
    ...
    favorites = relationship("FavoriteLogFile", back_populates="user", cascade="all, delete-orphan")
    search_history = relationship("SearchHistory", back_populates="user", cascade="all, delete-orphan")
    alerts = relationship("AlertRule", back_populates="user", cascade="all, delete-orphan")
    comments = relationship("LogEntryComment", back_populates="user", cascade="all, delete-orphan")
...
class LogFile(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)  # Dosyayı yükleyen kullanıcı
    category_id = Column(Integer, ForeignKey("categories.id"), nullable=True, index=True)  # Kategori
    ...
    analysis = relationship("LogAnalysis", back_populates="log_file", uselist=False, cascade="all, delete-orphan")
...
class LogAnalysis(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    log_file_id = Column(Integer, ForeignKey("log_files.id"), unique=True, nullable=False)
...
class SavedSearch(Base):
    ...
    search_params = Column(JSON, nullable=False)  # {"log_level": "ERROR", "search": "database", "file_id": 1, ...}
...
class AlertRule(Base):
    ...
    # Koşullar
    condition_type = Column(String(50), nullable=False)  # "error_count", "pattern_match", "threshold"
    condition_params = Column(JSON, nullable=False)  # {"threshold": 10, "time_window": 300, "log_level": "ERROR"}
    
    # Bildirim kanalları
    notification_channels = Column(JSON, nullable=False)  # ["email", "slack", "webhook"]
    recipients = Column(JSON, nullable=False)  # {"email": ["user@example.com"], "slack_webhook": "url"}
...
class SearchHistory(Base):
    ...
    search_params = Column(JSON, nullable=True)  # {"log_level": "ERROR", "file_id": 1, ...}
...
class Tag(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), unique=True, nullable=False, index=True)  # Tag adı (örn: "production", "backend")
...
class Category(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), unique=True, nullable=False, index=True)  # Kategori adı (örn: "Production", "Development")
    description = Column(Text, nullable=True)
    color = Column(String(7), default="#667eea")  # Hex renk kodu
