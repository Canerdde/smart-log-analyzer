...
from app.schemas import TagCreate, TagResponse, CategoryCreate, CategoryResponse, CategoryUpdate, BulkTagRequest
...
async def add_tags_to_file(
    ...
    if current_user and current_user.role != "admin":
        if log_file.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="Bu dosyayı düzenleme yetkiniz yok")
...
async def delete_category(
    ...
    db.query(LogFile).filter(LogFile.category_id == category_id).update({"category_id": None})
