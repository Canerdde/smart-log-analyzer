...
def _calculate_throughput(entries: List[LogEntry]) -> Dict[str, Any]:
    ...
    http_requests = sum(1 for e in entries if any(
        keyword in e.message.lower() 
        for keyword in ['http', 'request', 'get ', 'post ', 'put ', 'delete ', 'api']
    ))
...
def _detect_bottlenecks(entries: List[LogEntry], response_times: List[float]) -> List[Dict[str, Any]]:
    ...
    for entry in entries:
        ...
        if any(keyword in message for keyword in ['ms', 'duration', 'took', 'response']):
...
async def compare_performance(
    ...
    return {
        ...
        'best_performance': min(comparison, key=lambda x: x['avg_response_time_ms']) if comparison else None,
        'worst_performance': max(comparison, key=lambda x: x['avg_response_time_ms']) if comparison else None
    }
