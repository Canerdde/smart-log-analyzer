...
async def correlate_logs(
    ...
    time_window: int = Query(
        300, description="Zaman penceresi (saniye) - olaylar arası maksimum süre"
    ),
    correlation_type: str = Query(
        "temporal", description="Correlation tipi: temporal, pattern, error_chain"
    ),
    ...
    for file_id in file_ids:
        entries = (
            db.query(LogEntry)
            .filter(LogEntry.log_file_id == file_id, LogEntry.timestamp.isnot(None))
    ...
    if not all_entries:
        raise HTTPException(
            status_code=404, detail="Timestamp'li log girişi bulunamadı"
        )
...
def _temporal_correlation(
    entries: List[Dict], time_window: int, file_ids: List[int]
) -> Dict[str, Any]:
            ...
            correlations.append(
                {
                    ...
                    "time_span": (
                        related_events[-1]["timestamp"] - related_events[0]["timestamp"]
                    ).total_seconds(),
...
def _pattern_correlation(entries: List[Dict], file_ids: List[int]) -> Dict[str, Any]:
    ...
    cross_file_patterns.sort(
        key=lambda x: (x["file_count"], x["occurrences"]), reverse=True
    )
...
def _error_chain_correlation(
    ...
    analyzed_chains.sort(
        key=lambda x: (x["file_count"], x["chain_length"]), reverse=True
    )
...
async def get_event_chain(
    ...
    if current_user.role != "admin":
        ...
        if not file or file.user_id != current_user.id:
            raise HTTPException(
                status_code=403, detail="Bu dosyaya erişim yetkiniz yok"
            )
