...
async def create_saved_search(search: SavedSearchCreate, db: Session = Depends(get_db)):
    """
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"'{search.name}' adında bir arama zaten mevcut"
        )
...
async def use_saved_search(search_id: int, db: Session = Depends(get_db)):
    """
...
async def update_saved_search(
    search_id: int, search: SavedSearchCreate, db: Session = Depends(get_db)
):
    ...
    if search.name != saved_search.name:
        existing = (
            db.query(SavedSearch)
            .filter(SavedSearch.name == search.name, SavedSearch.id != search_id)
            .first()
        )
        if existing:
            raise HTTPException(
