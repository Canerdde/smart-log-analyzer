...
async def compare_logs(request: LogComparisonRequest, db: Session = Depends(get_db)):
    """
    ...
    analysis_1 = (
        ...
        .filter(LogAnalysis.log_file_id == request.file_id_1)
        .first()
    )
    analysis_2 = (
        db.query(LogAnalysis)
        .filter(LogAnalysis.log_file_id == request.file_id_2)
        .first()
    )
    ...
    errors_1 = (
        db.query(LogEntry)
        .filter(
            LogEntry.log_file_id == request.file_id_1, LogEntry.log_level == "ERROR"
        )
        .all()
    )
    ...
    errors_2 = (
        db.query(LogEntry)
        .filter(
            LogEntry.log_file_id == request.file_id_2, LogEntry.log_level == "ERROR"
        )
        .all()
    )
    ...
    for error in unique_to_file_1[:20]:  # İlk 20
        ...
        unique_to_file_1_with_count.append({"message": error, "count": count})
    ...
    for error in unique_to_file_2[:20]:  # İlk 20
        ...
        unique_to_file_2_with_count.append({"message": error, "count": count})
