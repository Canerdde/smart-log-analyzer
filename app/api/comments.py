...
@router.post("/entries/{entry_id}/comments", response_model=LogEntryCommentResponse, status_code=201)
async def create_comment(
    entry_id: int,
    comment: LogEntryCommentCreate,
...
@router.get("/entries/{entry_id}/comments", response_model=List[LogEntryCommentResponse])
async def get_entry_comments(
    ...
    comments = db.query(LogEntryComment).filter(
        LogEntryComment.log_entry_id == entry_id
    ).order_by(LogEntryComment.created_at.desc()).all()
...
async def update_comment(
    ...
    """Yorumu güncelle"""
    log_comment = db.query(LogEntryComment).filter(LogEntryComment.id == comment_id).first()
    if not log_comment:
        raise HTTPException(status_code=404, detail="Yorum bulunamadı")
