...
async def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    recent_files = db.query(LogFile).order_by(LogFile.uploaded_at.desc()).limit(10).all()
    
    # Hata trendi (son 10 dosyanın hata sayıları)
    recent_analyses = db.query(LogAnalysis).join(LogFile).order_by(
        LogFile.uploaded_at.desc()
    ).limit(10).all()
    ...
    error_trend = [
        {
            ...
            "uploaded_at": analysis.log_file.uploaded_at.isoformat() if analysis.log_file.uploaded_at else None
        }
