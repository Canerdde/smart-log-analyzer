...
async def add_favorite(
    favorite: FavoriteLogFileCreate,
    ...
    existing = db.query(FavoriteLogFile).filter(
        FavoriteLogFile.log_file_id == favorite.log_file_id
    ).first()
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Bu dosya zaten favorilerde"
        )
    
    fav = FavoriteLogFile(
        log_file_id=favorite.log_file_id,
...
async def list_favorites(
    ...
    favorites = db.query(FavoriteLogFile).order_by(
        FavoriteLogFile.created_at.desc()
    ).all()
    ...
    for fav in favorites:
        ...
        result.append(FavoriteLogFileResponse(
            id=fav.id,
            log_file_id=fav.log_file_id,
            notes=fav.notes,
            created_at=fav.created_at,
...
async def remove_favorite(
    ...
    """Favoriden kaldır"""
    favorite = db.query(FavoriteLogFile).filter(FavoriteLogFile.id == favorite_id).first()
    if not favorite:
