...
try:
    ...
    print(
        "Export modülü bulunamadı (reportlab/openpyxl eksik), PDF/CSV/Excel export devre dışı"
    )
...
async def export_pdf(file_id: int, db: Session = Depends(get_db)):
    ...
    if not EXPORT_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="PDF export mevcut değil. reportlab paketi gerekli."
        )
...
async def export_csv(file_id: int, db: Session = Depends(get_db)):
    """Analiz sonuçlarını CSV olarak export et"""
    if not EXPORT_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="CSV export mevcut değil. reportlab paketi gerekli."
        )
...
async def export_excel(file_id: int, db: Session = Depends(get_db)):
...
async def export_json(
    ...
    if include_analysis:
        analysis = (
            db.query(LogAnalysis).filter(LogAnalysis.log_file_id == file_id).first()
        )
