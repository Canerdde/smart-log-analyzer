...
from app.schemas import LogFileResponse, BulkDeleteRequest, BulkExportRequest, BulkFavoriteRequest
...
async def upload_log_file(
    ...
    try:
        ...
        if use_ai:
            ...
            print(f"AI analiz sonucu: ai_comment={ai_result.get('ai_comment') is not None}, ai_suggestions={ai_result.get('ai_suggestions') is not None}")
...
async def get_log_files(
    ...
    files = query.options(joinedload(LogFile.tags), joinedload(LogFile.category)).order_by(LogFile.uploaded_at.desc()).offset(skip).limit(limit).all()
    return files
...
async def get_log_file(
    ...
    from sqlalchemy.orm import joinedload
    log_file = db.query(LogFile).options(joinedload(LogFile.tags), joinedload(LogFile.category)).filter(LogFile.id == file_id).first()
    if not log_file:
        ...
        for file_id in request.file_ids:
            ...
            try:
                ...
                if request.format == "json":
                    export_response = export_logs_to_json(log_file, entries, request.include_analysis, analysis)
                else:
                    export_response = export_logs_to_xml(log_file, entries, request.include_analysis, analysis)
...
async def bulk_favorite_files(
    ...
    for file_id in request.file_ids:
        ...
        if request.action == "add":
            ...
            # Favoriden çıkar
            favorite = db.query(FavoriteLogFile).filter(
                FavoriteLogFile.log_file_id == file_id,
                FavoriteLogFile.user_id == current_user.id
            ).first()
