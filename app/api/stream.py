...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, file_id: int):
        ...
        print(f"WebSocket bağlantısı: file_id={file_id}, toplam={len(self.active_connections[file_id])}")
    ...
    try:
        ...
        if not log_file:
            await manager.send_personal_message(
                json.dumps({"error": "Log dosyası bulunamadı"}),
                websocket
            )
        ...
        while True:
            ...
            if log_level:
                new_entries = new_entries.filter(LogEntry.log_level == log_level.upper())
            if search:
