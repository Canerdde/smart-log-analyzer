...
async def get_log_entries(
    ...
    search: Optional[str] = Query(None, description="Mesaj içinde arama (normal veya regex)"),
    search_type: Optional[str] = Query("normal", description="Arama tipi: normal, regex"),
    ...
    and_conditions: Optional[str] = Query(None, description="AND koşulları (JSON string)"),
    or_conditions: Optional[str] = Query(None, description="OR koşulları (JSON string)"),
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db)
):
        ...
        try:
            ...
            if isinstance(conditions, dict):
                if "log_level" in conditions:
                    query = query.filter(LogEntry.log_level == conditions["log_level"].upper())
            ...
            if isinstance(conditions, dict):
                ...
                if "log_level" in conditions:
                    ...
                    if isinstance(levels, list):
                        or_filters.append(LogEntry.log_level.in_([l.upper() for l in levels]))
                    else:
...
async def get_errors(
    ...
    return await get_log_entries(file_id, log_level="ERROR", skip=skip, limit=limit, db=db)
...
async def get_warnings(
    ...
    return await get_log_entries(file_id, log_level="WARNING", skip=skip, limit=limit, db=db)

@router.get("/{file_id}/patterns")
async def get_patterns(
    file_id: int,
    min_similarity: float = Query(0.7, ge=0.0, le=1.0, description="Minimum benzerlik oranı"),
    db: Session = Depends(get_db)
):
    """
